  ...
  return (
    <>
      ...
      <td scope="row" className="mt-1 ms-3">
        ...
        <Greeting count={count} item={item.dataitem} id={item.id} onClickChange={item.onClickChange()}/>֏
      </td>
    </>
  );
}

const Greeting = memo(function Greeting({ count, item, id ,onClickChange}) {
  if (Math.sign(count) == -1) {
    ...
    onClickChange(Number(count) * item)
