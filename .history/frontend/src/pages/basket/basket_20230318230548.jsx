...
function Basket() {
  ...
  useEffect(() => {
    ...
    axios.post(`http://barmatoys.loc/api/get/basket`, fd).then((res) => {
      ...
      setPrice(persons.price);
      setCount(persons.count);
    });
  }, []);
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td scope="row" className="text-danger">
                      {/* {dataitem.price} */}
                      {Object.values(price[dataitem.id])[0]}
                      {t("taradram")}
                    </td>

                    <CountChange
                      id={item.toyId}
                      item={Object.values(count[dataitem.toyId])[0]}
                      key={item.toyId}
                      dataitem={Object.values(price[dataitem.id])[0]}
                      click={click}
                    />
      ...
      <div className="container d-flex align-items-end flex-column">
        ...
        >
          ...
          <div className="d-flex">
            {localStorage.getItem("click") == 1 ? <h2>{Subtotal}֏</h2>:""}
