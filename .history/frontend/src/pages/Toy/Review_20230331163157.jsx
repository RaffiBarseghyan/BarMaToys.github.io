...
export default function Clicks(id) {
  ...
  const submit = () => {
    if (stars == "") {
      ...
      axios.post(`http://barmatoys.loc/api/create/review`, formData);
      ...
      setTimeout(() => {
        window.location.replace(`/toy/${id.id}`);
      }, 1600);
    }
  };
  ...
  return (
    <div id="asd">
      <button onClick={handleShow} className={style.writeReview}>
        <img className="me-2" src={pencil} alt="" /> {t("WriteАРeview")}
      </button>
      {localStorage.getItem("loginEmail") ? (
        <>
          <Modal
            className={style.more1}
            show={show}
            onHide={handleClose}
          ...
          >
            ...
            <Modal.Body className={`${style.moreBody} container`}>
              <Modal.Title
                className={`${style.itemName1} d-flex flex-column align-items-center`}
              >
                <h2 className={`${style.h2Rev}`}>{t("Reviews")}</h2>
                {/* Toy Stars */}
                <div className={`d-flex align-items-center  ${style.ratings}`}>
                  <h3 className={`me-4 ${style.h3} d-flex align-items-center`}>
                    {t("Rating")} <p className={style.astxanish}>*</p>
                  </h3>
                ...
                <label className={`align-items-center  ${style.ratings2}`}>
                  <h3 className={`me-4 ${style.h3} d-flex align-items-center`}>
                    {t("Review")} <p className={style.astxanish}>*</p>
                  </h3>
                  <textarea
                    cols="30"
                    rows="10"
                ...
                <button
                  className={`${style.writeReview} mt-4`}
                  onClick={submit}
                >
                  {t("Submit")}
                </button>
