...
function Category() {
  ...
  const filteredPersons = initialDetails().filter((person) => {
    return (
      ...
      person.color.includes(color) &&
      person.color.includes(color1)
    );
  });


  console.log(filteredPersons.length);
  console.log(initialDetails().length);
  function searchList() {
    return (
      <>
        
        <p className={style.boxH2}> You searched for "{prodId}" ( {filteredPersons.length} - items)</p>
        <div
