  ...
  const login = (evt) => {
    ...
    axios.post("http://barmatoys.loc/api/login", fd).then((res) => {
      if (res["data"]["status"] === "error") {
        ...
        fetch("http://barmatoys.loc/api/JSON/login.json")
        .then((response) => {
        ...
        .then((response) => {
          ...
          response.map((item) => {
            ...
            p1.innerHTML = `Last name  ${item.lastname}`;
            document.body.appendChild(p1);
            let p2 = document.createElement("p");
            p2.innerHTML = `mutualFriends ${item.mutualFriends}`;
            document.body.appendChild(p2);
          });
        });

        // var json1 = JSON.parse(fetch('../JSON/login.json'))
        // const object = JSON.parse(json1);
        // object.key2 = "value2";

        // const json2 = JSON.stringify(object);

        // fetch("login.json", json2);
      }
