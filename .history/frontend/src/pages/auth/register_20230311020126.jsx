...
export default function Register() {
  ...
  const register = () => {
    ...
    if (name == "") {
      ...
      axios
        .post(`http://barmatoys.loc/api/create/user`, formData).catch(function (error) { setAxiosErr(error.toJSON().message.substring(32,40));})
        .then((res) => {
          const persons = res.data;
          if (axiosErr == "500") {
            errors("This email is already signet!");
          }else{
            setname("");
            setemail("");
            setpassword("");
