...
class BasketController extends Controller
{
    public function basket(Request $req)
    {
        ...
        $data = Basket::find($req->post('userEmail'));
        $data = Basket::find($req->post('toyId'));

        $data1 = Basket::where('userEmail', $req->post('userEmail'))::where('toyId', $req->post('toyId'))->update();
        ...
        if (Basket::find($req->post('userEmail'))  && Basket::find($req->post('toyId'))) {
            ...
            $dataCount = Basket::where('count', $req->post('count'))->get();
